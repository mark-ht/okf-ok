...
Use it with any Agent Skills-compatible tool by adding this repository's `skills/` directory to that tool's skill search path, or by copying the entire `okf-lint/` directory without renaming it. The only runtime dependency is `okflint` (or the documented digest-pinned container image).
